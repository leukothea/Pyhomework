...
class Grid(Deck):
        ...
        def cardCompare(self, i, n ):
            ...
            if i in range(4) and n in range(4):
                    ...
                    # Look for an image match in the card to the right. If there is one, add it to the hits list.
                    hithoriz = [(w) for w in (right) if w in (mainimages)]
                    if hithoriz:
                        self.hits.extend(hithoriz)
...
print "Welcome... to Madame Ykaterina's Fortunetelling Shoppe. Cross my palm with silver, and I shall reveal unto you your fate in love, loss, and life! \n Ah, you want to examine the cards? Here they are, curious stranger. \n"
print deck
...
print "\nFirst, we lay out the cards in a five-by-five grid. Patience, as many things will shortly be revealed! \n"
print grid.array
...
for item in grid.hits:
    for line in grid.rowmatches:
        if line[0] in grid.hits:
            if line[1] == 'clock':
                result = line[0] + '1'
