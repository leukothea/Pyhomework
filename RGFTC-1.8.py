...
# Dictionary of possible readings. There are 25 cards, each with 4 meanings. The picture + position
...
"Fish2" : "If you want su            self.hits = []ccess, seek it on the water and not on land.", 
...
class Grid(Deck):
        ...
        def cardCompare(self, i, n ):
            # get the images on the cards to be compared and cast the values as tuples. 
            ...
            if i in range(4) and n in range(4):
                    ...
                    main = tuple(main[0])
                    right = self.getCard(i, n+1)
                    right = tuple(right[0])
                    beneath = self.getCard(i+1, n)
                    beneath = tuple(beneath[0])
                    hithoriz = [(w) for w in (right) if w in (main)] 
                    if hithoriz:
                        self.hits.extend(hithoriz)
                    hitbeneath = [(w) for w in (beneath) if w in (main)]
                    if hitbeneath:
            ...
            # for main cards in row 4, find only the card to the right
                    ...
                    right = self.getCard(i, n+1)
                    right = tuple(right[0])
                    hithoriz2 = [(w) for w in (right) if w in (main)]
                    if hithoriz2:
            ...
            # for main cards in column 4, find only the card beneath
                    ...
                    beneath = self.getCard(i+1, n)
                    beneath = tuple(beneath[0])
                    hitbeneath2 = [(w) for w in (beneath) if w in (main)]
                    if hitbeneath2:
