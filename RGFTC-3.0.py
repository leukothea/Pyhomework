...
# First, define 25 card objects with 25 tuples, each composed of four picturehalves. 
...
# Dictionary of possible readings to use in the final step. 
# There are 25 cards, each with 4 meanings. 
# The picture + position is the key, and its meaning is the value. 
...
'Fox3' : 'It is not wise to trust a new friendship.', 
...
'Bear4' : 'Do not have faith in every piece of advice.', 
...
class Card(object):
    # to define the cards and the card-specific methods
...
class Deck(object): 
    # a collection of 25 cards (to ensure uniqueness)
    def __init__(self):
        self.cards = []
...
class Grid(Deck):
        # an empty 5x5 grid to hold the 25 cards, and various lists to hold data
        ...
        def fill(self):
            # shuffle cards, then fill the grid with unique cards
            self.array = []
        ...
        def getCard(self, i, n):
            # gets the whole card at position i, n
            card = self.array[i][n]
            return card

        def getImages(self, i, n):
            # gets the card at position i, n, using card class method directionStrip to return images only
            images = self.array[i][n].directionStrip()
            return images
        
        def cardCompare(self, i, n ):
            # Get the images on the cards to be compared. Cast the values as tuples for comparison. 
            # This case covers the bulk of the grid: cards that have both a right-adjacent and beneath card.
            if i in range(4) and n in range(4):
                    # get the images in the main card, the card to the right, and the card beneath.
                    main = self.getCard(i, n)

                    mainimages = self.getImages(i, n)
                    mainimages = tuple(mainimages[0])
                    right = self.getImages(i, n+1)
                    right = tuple(right[0])
                    beneath = self.getImages(i+1, n)
                    beneath = tuple(beneath[0])

                    # Look for an image match in the card to the right. If there is one, add it to the row hits list.
                    hithoriz = [(w) for w in (right) if w in (mainimages)]
                    if hithoriz:
                        self.rowhits.extend(hithoriz)

                    # Also if there's a row match, add the main card's picturehalves to a list of row matches.
                    # I really wanted to just add the CORRECT image+direction to the list, but could not figure out how.
                    # So I added them all, to attempt to screen out the right ones later by comparing with grid.hits.
                    # An attempt that would by and large prove fruitless. 
                    picturehalves = (main.picturehalves)
                    if hithoriz:
                        self.rowmatches.extend(picturehalves)

                    # Look for an image match in the card beneath. If there is one, add it to the hits list.
                    hitbeneath = [(w) for w in (beneath) if w in (mainimages)]
                    if hitbeneath:
                        self.columnhits.extend(hitbeneath)

                    # Also if there's a match, add the main card's picturehalves to the list of column matches.
                    picturehalves = (main.picturehalves)
                    if hitbeneath:
                        self.columnmatches.extend(picturehalves)

            # for main cards in row 4, find only the card to the right (because there are no cards beneath)
            elif i == 4 and n in range(4):
                    main = self.getCard(i, n)
                    mainimages = self.getImages(i, n)
                    mainimages = tuple(mainimages[0])
                    right = self.getImages(i, n+1)
                    right = tuple(right[0])
                    hithoriz2 = [(w) for w in (right) if w in (mainimages)]
                    if hithoriz2:
                        self.rowhits.extend(hithoriz2)
            ...
            # for main cards in column 4, find only the card beneath (because there are no cards to the right)
            elif n == 4 and i in range(4):
                    main = self.getCard(i, n)
                    mainimages = self.getImages(i, n)
                    mainimages = tuple(mainimages[0])
                    beneath = self.getImages(i+1, n)
                    beneath = tuple(beneath[0])
                    hitbeneath2 = [(w) for w in (beneath) if w in (mainimages)]
                    if hitbeneath2:
                        self.columnhits.extend(hitbeneath2)
        ...
        def cleanup(self):
            for item in self.columnhits:
                if item[0] in self.rowhits:
                    self.columnhits.remove(item)
                else:
                    return self.columnhits

                       
# -- Processing -- #

# Populate the deck with the ListOfCards (defined at the beginning of the file)
...
print "Welcome... to Madame Ykaterina's Fortunetelling Shoppe. \nCross my palm with silver, and I shall reveal unto you your fate in love, money, and life! \n Ah, you want to examine the cards? Here they are, curious stranger. \n"
print deck
...
print "\nFirst, I shall lay out the cards in a five-by-five grid. Patience, as many things will shortly be revealed! \n"

print grid.array


# Starting at the top left, compare the card to the immediate right and immediately beneath it for 
# a match in picturehalves. Don't evaluate the final card (position 4, 4). 
...
# Give the list of matches (for QA purposes)

print "\nLet me gaze upon the cards... searching for the images that will spell your future. Ah, I see them now!\n"

grid.cleanup()
hits = grid.rowhits + grid.columnhits
print hits

# Now, find the directionality of the arrow in the image that's a match. 

# If the match occurred in a ROW, and the main card's image's direction is "clock" for clockwise, the arrow direction is '1', 
# which indicates DOWN. 

# If the match occurred in a ROW, and the main card's image's direction is "widdershins" for counterclockwise, the arrow
# direction is '4', which indicates UP. 

# If the match occurred in a COLUMN, and the main card's image's direction is "clock," the arrow direction is '2', which
# indicates RIGHT. 

# If the match occurred in a COLUMN, and the main card's image's direction is "widdershins," the arrow direction is '3', which
# indicates LEFT. 

# The numbers are added to the imagenames, and the results saved in a list claled "readings."

readings = []

for line in grid.rowmatches:
    if line[0] in grid.rowhits:
        if line[1] == 'clock':
            result = line[0] + '1'
            readings.append(result)
        elif line[1] == 'widdershins':
            result = line[0] + '4'
            readings.append(result)
        else:
            break

for line in grid.columnmatches:
    if line[0] in grid.columnhits:
        if line[0] in grid.rowmatches:
            break
        elif line[1] == 'clock':
            result = line[0] + '2'
            readings.append(result)
...
print "\nAnd now we can find the directions of the arrows. Yes, yes, I see them through the swirling mists of fate.\n"

print Set(readings)

# Finally, look up those readinds in the "Meaning" dictionary and return the list

print "\n And now, to find the meanings of those images in context of your entire life as it currently stands. Only you will know whether the cards speak truly, but Madame Ykaterina can assure you that they have never yet led her astray! \n"

for line in Set(readings):
    lookup = (line)
    lookup2 = lookup + " : " + Meaning.get(lookup)
    lookup3 = ([lookup2])
    lookup4 = list(Set(lookup3))
    print lookup4

print "\nI also see great things for you in the field of computer programming... but that is a reading for another day!"
